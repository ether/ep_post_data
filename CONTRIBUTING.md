# Contributing to ep_post_data

Thanks for helping improve `ep_post_data` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_post_data` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_post_data from your local clone
pnpm run plugins i --path ../ep_post_data

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_post_data/` are picked up after a server restart.
...
* Etherpad version, ep_post_data version, browser + OS
* Steps to reproduce
